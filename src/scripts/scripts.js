...
let pokemonRepository = (() => {
  ...
  function handleMove(e) {
    ...
    if (Math.abs(diffX) > Math.abs(diffY)) {
      ...
      if (diffX > 0) {
        if (currPokeIndex == 0) {
          //if first element in pokemonList array continue to display first pokemon on swipe left
        ...
        if (currPokeIndex == pokemonList.length - 1) {
          ...
          //if last element in pokemonList array continue to display last pokemon on swipe right
          showDetails(pokemonList[currPokeIndex + 1]);
        }
      }
    }
